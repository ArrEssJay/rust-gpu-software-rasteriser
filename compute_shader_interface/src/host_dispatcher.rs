...
pub fn execute_compute_shader_host(
    scaled_vertex_buffers: VertexBuffers,
    raster_parameters: &RasterParameters,
) -> Vec<f32> {
    
    let raster_size = (raster_parameters.scaled_raster_size() * raster_parameters.scaled_raster_size()) as usize;
    let unsafe_raster: SyncUnsafeCell<Vec<f32>> = vec![f32::NAN; raster_size].into();
    let raster_size_cells = raster_parameters.scaled_raster_size() / GRID_CELL_SIZE_U32;
    ...
    let aabb: Vec<AABB> = (0..raster_parameters.triangle_count as usize)
        .map(|i| {
            compute_aabb(
                i,
                scaled_vertex_buffers.u,
                scaled_vertex_buffers.v,
                scaled_vertex_buffers.indices,
            )
        })
        .collect();

    // Parallel iterate over cell rows
    (0..raster_size_cells).into_par_iter().for_each(|cell_y| {
        for cell_x in 0..raster_size_cells {
            ...
            load_cell_triangles(
                scaled_vertex_buffers.u,
                scaled_vertex_buffers.v,
                scaled_vertex_buffers.attribute,
                scaled_vertex_buffers.indices,
                aabb.as_slice(),
                cell,
                &mut cell_data,
...
mod tests {
    ...
    fn test_execute_compute_shader_host() {
        ...
        assert_eq!(result.len(), (params.scaled_raster_size() * params.scaled_raster_size()) as usize);
